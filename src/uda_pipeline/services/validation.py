...
def sanitize_result(result: ExtractionResult) -> ExtractionResult:
    ...
    for metric in result.metrics:
        _apply_result_period(metric, result)
    ...
    if issues:
        ...
        for metric in result.metrics:
            if not _should_drop_metric(metric):
...
def _apply_result_period(metric: ExtractedMetric, result: ExtractionResult) -> None:
    metric.year = metric.year or result.reference_year
    metric.quarter = metric.quarter or result.reference_quarter


def _should_drop_metric(metric: ExtractedMetric) -> bool:
    if metric.metric_code not in NON_PERCENT_METRICS:
        return False
    excerpt = (metric.source_excerpt or "").lower()
    return _looks_like_percentage(metric.value, excerpt)
